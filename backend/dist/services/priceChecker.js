 ...
 */
function generateDatePairs(windowStart, windowEnd, durationDays) {
    const pairs = [];
...
async function checkAllAlerts() {
    const { rows: alerts } = await pool_1.pool.query(`
    SELECT a.*, u.email
    FROM alerts a
...
async function checkAlert(alert) {
    const pairs = generateDatePairs(new Date(alert.window_start), new Date(alert.window_end), alert.trip_duration_days);
    let bestResult = null;
    for (const pair of pairs) {
        try {
            const result = await (0, duffel_1.searchCheapestOffer)(alert.origin, alert.destination, pair.departure, pair.return);
    ...
    await pool_1.pool.query(`INSERT INTO price_history (alert_id, departure_date, return_date, price_usd, offer_id)
     VALUES ($1, $2, $3, $4, $5)`, [alert.id, bestResult.departure, bestResult.return, bestResult.price, bestResult.offerId]);
    ...
    if (bestResult.price <= Number(alert.max_price_usd)) {
        await (0, notifications_1.sendPriceAlert)({
            toEmail: alert.email,
            origin: alert.origin,
